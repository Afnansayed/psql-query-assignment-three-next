
-- query 1
select 
  b.booking_id,
  u.name as customer_name,
  v.name as vehicle_name,
  b.start_date,
  b.end_date,
  b.status
from
  bookings b 
  inner join users u on b.user_id = u.user_id
  inner join vehicles v on b.vehicle_id = v.vehicle_id
  ;


-- query 2
select v.*
from vehicles v
where not exists (select 1 from bookings b where b.vehicle_id = v.vehicle_id)
order by vehicle_id;

-- query 3
select * from vehicles 
where type = 'car' and status = 'available';


-- query 4
select
v.name as vehicles_name,
count(*) as total_bookings
from bookings b
inner join vehicles v on b.vehicle_id = v.vehicle_id
group by v.name
having count(*) > 2;






--  database create 
create database vehicle_rental_system;
-- create a enum for user role
create type user_role as enum ('admin', 'customer');

-- user table create
create table users(
  user_id serial primary key,
  name varchar(50) not null,
  phone bigint not null,
  email varchar(50) unique not null,
  password varchar(20) not null,
  role user_role default('admin') not null  
);


insert into users (name, email, phone, password, role) values
('Alice', 'alice@example.com', '1234567890','123456', 'customer'),
('Bob', 'bob@example.com', '0987654321','123456', 'admin'),
('Charlie', 'charlie@example.com', '1122334455','123456', 'customer');


-- create enums for type and status
create type vehicle_type as enum ('car', 'bike', 'truck');
create type vehicle_status as enum ('available', 'rented' ,'maintenance');

-- create vhicle table
create table vehicles (
  vehicle_id serial primary key,
  name varchar(50) not null,
  type vehicle_type not null,
  model varchar(50),
  registration_number varchar(50) unique not null,
  rental_price decimal(10,2) not null,
  status vehicle_status not null
);


insert into vehicles (name, type, model, registration_number, rental_price, status) values
('Toyota Corolla', 'car', '2022', 'ABC-123', 50.00, 'available'),
('Honda Civic', 'car', '2021', 'DEF-456', 60.00, 'rented'),
('Yamaha R15', 'bike', '2023', 'GHI-789', 30.00, 'available'),
('Ford F-150', 'truck', '2020', 'JKL-012', 100.00, 'maintenance');

-- create enum for status
create type booking_status as enum ('pending', 'confirmed', 'completed', 'cancelled');

-- create booking taqble
create table bookings (
  booking_id serial primary key,
  user_id int not null,
  vehicle_id int not null,
  start_date date not null,
  end_date date not null,
  status booking_status not null default 'pending',
  total_cost decimal(10,2) not null,

  constraint fk_bookings_user
    foreign key (user_id)
    references users(user_id)
    on delete cascade,

  constraint fk_bookings_vehicle
    foreign key (vehicle_id)
    references vehicles(vehicle_id)
    on delete cascade
);


-- Insert bookings data exactly as shown in the image
insert into bookings (user_id, vehicle_id, start_date, end_date, status, total_cost) values
(1, 2, '2023-10-01', '2023-10-05', 'completed', 240.00),
(1, 2, '2023-11-01', '2023-11-03', 'completed', 120.00),
(3, 2, '2023-12-01', '2023-12-02', 'confirmed', 60.00),
(1, 1, '2023-12-10', '2023-12-12', 'pending', 100.00);





